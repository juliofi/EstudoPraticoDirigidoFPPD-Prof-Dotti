...
func busca(r *Nodo, val int) bool {
	if r != nil {
		return (r.v == val) || busca(r.e, val) || busca(r.d, val)
	}
	return false
}
...
func buscaConc(r *Nodo, val int, ret chan bool) {
	if r != nil {
		ret2 := make(chan bool)
		go buscaConc(r.e, val, ret2)
		go buscaConc(r.d, val, ret2)
		ret <- ((r.v == val) || <-ret2 || <-ret2)
	}
	ret <- false
}
...
func retornaParImpar(r *Nodo, saidaP chan int, saidaI chan int, fin chan struct{}) {
	if r != nil {
		...
		go retornaParImpar(r.e, saidaP, saidaI, fin2)
		go retornaParImpar(r.d, saidaP, saidaI, fin2)
		if r.v%2 == 0 {
			saidaP <- r.v
		} else {
			saidaI <- r.v
